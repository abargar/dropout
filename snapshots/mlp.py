...
def takeSnapshot(classifier, counter):
    classifier_params = [ param.get_value() for param in classifier.getParams() ]
    numLayers = len(classifier_params)
    for i in range(numLayers):
        params_file = open('snapshots/dropout/params_{0}_layer_{1}.txt'.format(counter, i), 'wb')
        layer = classifier_params[i]
        numRows = layer.shape[0]
        try:
            param_str = '%f,'*layer.shape[1]
            param_str = param_str[:-1]
            for j in range(numRows):
                params_file.write(', '.join('%f' %F for F in layer[j,:]))
                params_file.write(';\n')
        except IndexError:
            if numRows == 1:
                params_file.write(layer)
            else:
                for j in range(numRows):
                    try:
                        params_file.write(', '.join('%f' % F for F in layer[j]))
...
def test_mlp(
    ...
    datasets = load_mnist(dataset, classes)
    ...
    while epoch_counter < n_epochs:
        ...
        # snapshot parameters
        if epoch_counter % snapshot_counter == 0:
            takeSnapshot(classifier, epoch_counter)
